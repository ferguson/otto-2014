...
module.exports = global.otto.db = do ->  # note the 'do' causes the function to be called
  ...
  db.connect = (database='otto', url='mongodb://localhost:27017', callback=no) ->
    ...
    mongo_client = new mongodb.MongoClient(url, {safe:true, strict:false})

    mongo_client.connect (err, p_client) ->   # FIXME i suspect p_client might not be a thing now that we've upgraded the mongodb module from 1.2.0 to 3.2.5
      if err
        if callback then callback "error trying to open database #{database} on #{url}: #{err}"
        return
      mongo = mongo_client.db(database);
      attach_collections collections_inuse, ->
        c.objects.count (err, count) ->
          ...
          console.log "connected to database #{database} on #{url}"
