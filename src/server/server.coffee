...
#  @use cookie_parser()
  ...
  @app.use '/static', @express.static(otto.OTTO_ROOT + '/static')
  @app.use serve_favicon(otto.OTTO_ROOT + '/static/images/favicon.ico')
  ...
  @get '/download/:id': ->
    ...
    jsonreq.get 'http://localhost:8778/load_lists?objects=1', (err, data) =>
      ...
      if archivename
        ...
        zip = new zip_stream level: 1
        zip.pipe( @res )
        # FIXME this is all probably broken because of the shift from zipstream to zip-stream
        nextfile = =>
          if filenames.length
...
#  socket_get_sessionip = (s) -> otto.sessioniplist[s.id]
  socket_get_session = (s) -> s.session
  socket_get_sessionip = (s) -> s.session
